...
By the way, these messages are consistent with the error and warning messages that are shown when using validation functions directly on data (e.g., `small_table %>% expect_col_vals_between(...)`).
...
Should the evaluation of the expression not work out (for example, the non-existent `d` column was used in the above) you'll get an `NA` value returned. If using an agent, the report will show that the evaluation was unsuccessful (look for the explosion in the `EVAL` column).
...
Ces messages sont cohérents avec les messages d'erreur et d'avertissement qui s'affichent lors de l'utilisation de fonctions de validation directement sur les données (par exemple, `small_table %>% expect_col_vals_between (...)`).
...
Übrigens stimmen diese Meldungen mit den Fehler- und Warnmeldungen überein, die angezeigt werden, wenn Validierungsfunktionen direkt für Daten verwendet werden (z. B. `small_table %>% expected_col_vals_between (...)`).
